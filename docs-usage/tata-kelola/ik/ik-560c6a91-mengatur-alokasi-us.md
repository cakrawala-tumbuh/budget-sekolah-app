---
title: "IK Mengatur Alokasi US"
description: "Instruksi kerja bagi Pengelola Anggaran Cabang/Pusat untuk menambah, mengedit, dan menghapus alokasi komponen biaya US ke unit-unit yang dinaungi, dengan proporsi otomatis berbasis total siswa pada aplikasi Budget YPII."
published: true
...
# IK Mengatur Alokasi US

Instruksi Kerja Mengatur Alokasi US ini memandu Pengelola Anggaran Cabang/Pusat mendistribusikan
komponen biaya Uang Sekolah (US) dari organisasi CABANG atau PUSAT ke unit-unit yang dinaungi pada
halaman Alokasi US aplikasi Budget YPII. Komponen yang dialokasikan akan menaikkan tarif Uang Sekolah
di sisi unit penerima.

## Tujuan

Menghasilkan distribusi komponen biaya US dari Cabang/Pusat ke setiap unit penerima dengan proporsi
yang benar, sehingga simulasi tarif Uang Sekolah unit memperhitungkan alokasi biaya secara akurat dan
total distribusi selalu genap 100%.

## Prasyarat

Sebelum menjalankan Instruksi Kerja Mengatur Alokasi US, pelaksana telah login sebagai pengguna
organisasi bertipe CABANG atau PUSAT, komponen biaya yang akan dialokasikan telah tercatat di
organisasi tersebut, dan jumlah total siswa tiap unit telah tersedia sebagai dasar proporsi otomatis.

## Peralatan / Akses yang Dibutuhkan

Instruksi Kerja Mengatur Alokasi US dijalankan pada aplikasi Budget YPII, halaman **Alokasi US**
(`/organizations/{id}/alokasi-us`), oleh Pengelola Anggaran Cabang/Pusat dengan akses ke organisasi
CABANG atau PUSAT yang bersangkutan. Fitur ini hanya tersedia untuk organisasi bertipe CABANG dan
PUSAT.

## Instruksi

### 1. Membuka halaman Alokasi US

Langkah membuka halaman Alokasi US dimulai dari halaman detail organisasi.

1. Buka halaman detail organisasi CABANG atau PUSAT.
1. Klik kartu **Alokasi US** di bagian navigasi fitur.

### 2. Memahami tabel alokasi

Tabel alokasi menampilkan satu baris per unit penerima. Kolom-kolom yang ditampilkan pada tabel
Alokasi US diuraikan berikut ini.
...
| **Proporsi Otomatis** | Proporsi yang dihitung otomatis dari sisa porsi setelah dikurangi total semua override, berdasarkan total siswa unit |
...
sisa (100% dikurangi total semua override) yang dibagi proporsional berdasarkan total siswa unit
...
Penambahan alokasi US menentukan komponen biaya Cabang/Pusat yang masuk ke perhitungan tarif Uang
Sekolah unit.
...
### 4. Mengedit alokasi US

Pengeditan alokasi US dilakukan pada entri yang sudah tercatat.
...
### 5. Menghapus alokasi US

Penghapusan alokasi US mengeluarkan komponen biaya dari distribusi ke unit.
...
Setelah Instruksi Kerja Mengatur Alokasi US selesai, komponen biaya yang dialokasikan muncul di
halaman simulasi Uang Sekolah setiap unit penerima sebagai "Alokasi biaya dari Cabang" atau "Alokasi
...
- **Total Persentase Final tidak sesuai harapan** — periksa unit yang mengisi Override; sisa porsi
  (100% dikurangi total override) dibagi proporsional ke unit tanpa override berdasarkan total siswa.
- **Alokasi tidak muncul di simulasi unit** — pastikan alokasi telah tersimpan; komponen yang
  dialokasikan baru tampil di simulasi US unit penerima setelah entri disimpan.
