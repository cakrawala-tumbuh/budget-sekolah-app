...
const allocSchema = z.object({
  override_pct_us: z.string().optional(),
});
...
function AllocUnitForm({
  ...
  const { register, handleSubmit } = useForm<AllocValues>({
    ...
    defaultValues: {
      override_pct_us: currentOverride != null ? String(currentOverride * 100) : "",
    },
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="space-y-1.5">
        <Label>Override Proporsi US (%)</Label>
        <Input
          type="number"
          step="0.01"
          min={0}
          max={100}
          placeholder="Kosongkan = proporsional otomatis dari total siswa"
          {...register("override_pct_us")}
        />
        <p className="text-xs text-muted-foreground">
          Kosongkan agar proporsi dihitung otomatis berdasarkan total siswa.
        </p>
      </div>
...
export default function AlokasiUSPage({ params }: Props) {
  const { id } = use(params);
  ...
  const usPeaList = peaList?.filter((p) => !p.affects_up) ?? [];
  ...
  // Hitung total siswa dari semua unit untuk kolom Proporsi Otomatis (referensi mentah)
  ...
  const sumOverrideUs = (allocations ?? []).reduce(
    (sum, a) => sum + (a.override_pct_us ?? 0), 0
  );
  const remainingUs = Math.max(0, 1 - sumOverrideUs);
  const autoTotalStudentsTotal = childUnits.reduce((sum, unit) => {
    const alloc = allocations?.find((a) => a.from_organization_id === unit.id);
    if (alloc?.override_pct_us != null) return sum;
    const n = alloc?.total_students ?? assumptionMap.get(unit.id)?.total_students ?? 0;
    return sum + n;
  }, 0);
  ...
  const handleSaveAlloc = async (unitId: number, v: AllocValues) => {
    ...
    const pctUs = v.override_pct_us ? parseFloat(v.override_pct_us) / 100 : null;
    const payload: ContributionAllocationCreate = {
      ...
      override_pct_us: pctUs,
      override_pct_up:
        allocations?.find((a) => a.from_organization_id === unitId)?.override_pct_up ?? null,
    };
  ...
  const handleCopyFrom = async () => {
    ...
    const result = await copyPea.mutateAsync({ sourceOrgId: Number(copySourceOrgId), affectsUp: false });
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-6 flex flex-wrap items-start gap-3 sm:flex-nowrap sm:items-center">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold">Alokasi US</h1>
            <Badge variant="secondary">{org.name}</Badge>
          </div>
          <p className="text-sm text-muted-foreground">
            Konfigurasi komponen biaya Uang Sekolah yang dialokasikan ke unit-unit
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-lg font-semibold flex items-center gap-2">
              ...
              Kategori Biaya (Komponen US)
            </h2>
            <p className="text-sm text-muted-foreground">
              Kategori biaya dari organisasi ini yang akan dialokasikan ke unit sebagai komponen US.
            </p>
          </div>
        ...
        {loadingPea ? (
          ...
          <div className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">
            Belum ada kategori biaya US yang dikonfigurasi.
          </div>
        ) : (
          <Table>
            <TableHeader>
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-lg font-semibold flex items-center gap-2">
              ...
              Proporsi per Unit (Total Siswa US)
            </h2>
            <p className="text-sm text-muted-foreground">
            ...
            <TableBody>
              {childUnits.map((unit) => {
                ...
                const finalPct = hasOverride
                  ? alloc!.override_pct_us!
                  : autoTotalStudentsTotal > 0
                    ? remainingUs * (totalStudents / autoTotalStudentsTotal)
                    : 0;
                return (
      ...
      <Dialog open={showAddCategory} onOpenChange={setShowAddCategory}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Tambah Kategori Biaya US</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={showCopyDialog} onOpenChange={(open) => { setShowCopyDialog(open); if (!open) setCopyResult(null); }}>
        <DialogContent>
          ...
          {copyResult ? (
            ...
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Pilih organisasi sumber. Kategori US yang sudah ada tidak akan digandakan.
              </p>
              <div className="space-y-1.5">
      ...
      <Dialog open={!!editingUnitId} onOpenChange={(open) => !open && setEditingUnitId(null)}>
        <DialogContent>
          ...
          {editingUnit && (() => {
            ...
            const sumOthersUs = sumOverrideUs - (editingAlloc?.override_pct_us ?? 0);
            const remainingForEdit = Math.max(0, 1 - sumOthersUs);
            const autoStudentsForEdit = autoTotalStudentsTotal + (editingAlloc?.override_pct_us != null ? editingTotalStudents : 0);
            const editingAutoPct = autoStudentsForEdit > 0
              ? remainingForEdit * (editingTotalStudents / autoStudentsForEdit)
              : 0;
            return (
              <AllocUnitForm
