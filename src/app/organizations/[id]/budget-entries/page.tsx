  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        <Input placeholder="contoh: 24 × 12 × Rp 3.500.000" {...register("basis")} />
      </div>
...
export default function BudgetEntriesPage({ params }: Props) {
  const { id } = use(params);
  ...
  // Only operational categories
  const opCategories = (allCategories ?? []).filter((c) => c.is_operational);
  const grouped = groupByCategory(entries ?? [], opCategories);
    ...
    <div className="p-4 md:p-8">
        ...
        <>
          {grouped.length === 0 && !isLoading && (
            <div className="text-center py-16 text-muted-foreground">
              Belum ada entri biaya operasional. Klik &quot;Tambah Entri&quot; untuk mulai.
            </div>
          )}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Tambah Entri Biaya Operasional</DialogTitle>
          </DialogHeader>
          <EntryForm
      ...
      <Dialog open={!!editItem} onOpenChange={(o) => !o && setEditItem(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Edit Entri Biaya</DialogTitle>
          </DialogHeader>
          {editItem && (
            <EntryForm
              categories={opCategories}
